...
function selectAnswer(answer: string) {
  ...
  learningStore.recordAnswer(subject, chapterId, currentQuestion.value!.id, currentQuestion.value!.question, answer, currentQuestion.value!.answer)
}
  ...
  <div class="chapter-page">
    ...
    <div class="question-area" v-if="currentQuestion && !showCompletion">
      ...
      <div v-if="currentQuestion.type === 'fill'" class="fill-area">
        <div v-if="!showResult" class="fill-options">
          <button
            v-for="opt in currentQuestion.options"
            :key="opt"
            class="option-btn"
        ...
        <div v-if="!showResult && !currentQuestion.options?.length" class="fill-input">
          <input v-model="selectedAnswer" placeholder="输入答案" @keyup.enter="selectAnswer(selectedAnswer)" />
          <button class="btn btn-primary" @click="selectAnswer(selectedAnswer)">确认</button>
        </div>
      </div>
...
.fill-input { display: flex; gap: 12px; justify-content: center; }
.fill-input input { padding: 12px 16px; border: 2px solid #EEE; border-radius: 12px; font-size: 18px; font-family: inherit; width: 150px; text-align: center; }
.fill-input input:focus { outline: none; border-color: var(--color-primary); }
...
.next-btn { min-width: 180px; }
.btn { padding: 12px 24px; border: none; border-radius: 12px; font-size: 16px; font-weight: 700; cursor: pointer; font-family: inherit; transition: all 0.2s; }
